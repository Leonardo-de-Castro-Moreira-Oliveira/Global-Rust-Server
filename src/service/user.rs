...
pub async fn get_all_users(data: Data<AppState>) -> HttpResponse {
    match query_as!(UserSchema, "SELECT * FROM rust_user")
        ...
        .await
    {
        Ok(users) => HttpResponse::Ok().json(Success::new("success", users)), // Retornando a lista de sucesso.
        Err(err) => HttpResponse::InternalServerError().json(ServerError::from_sqlx_error(err)), // Retornando o erro inesperado pelo sqlx,
    }
}
...
pub async fn find_one_user(path: Path<String>, data: Data<AppState>) -> HttpResponse {
    ...
    match Uuid::parse_str(&path) {
        Ok(uuid) => match query_as!(UserSchema, "SELECT * FROM rust_user WHERE id = $1", uuid)
            ...
            .await
        {
            Ok(user) => HttpResponse::Ok().json(Success::new("found", user)), // Retornando o usuário que corresponde ao UUID.
...
pub async fn find_some_users(path: Path<String>, data: Data<AppState>) -> HttpResponse {
    ...
    .await
    {
        Ok(users) => {
            ...
            if !users.is_empty() {
                HttpResponse::Ok().json(Success::new("success", users)) // Retornando os usuaŕios encontrados.
            } else {
...
pub async fn add_one_user(body: Json<UserModel>, data: Data<AppState>) -> HttpResponse {
    ...
    .await
    {
        Ok(user) => HttpResponse::Created().json(Success::new("created", user)), // Retornando o usuario que foi criado.
        Err(err) => HttpResponse::InternalServerError().json(ServerError::from_sqlx_error(err)), // Retornando o erro inesperado pelo sqlx.
    }
}
